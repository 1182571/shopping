				...
				<div class="bread">
					...
					<ul class="fl sui-tag">
						...
						<li class="with-x" v-if="searchParams.categoryName">
							{{ searchParams.categoryName }}<i @click="removeCategroyName">×</i>
						</li>
						<!-- 关键字面包屑 -->
...
export default {
	...
	beforeMount() {
		...
		// 再发请求之前,把接口所需要的参数进行整理(服务器就会返回查询的数据)
		Object.assign(this.searchParams, this.$route.query, this.$route.params);
	},

	// 放这里只会挂在时候触发一次,封装成一个函数,(多次发送,监听路由变化)
	mounted() {
		// 再发请求之前带给服务器参数，用到了beforceMount钩子,searchParams发生变化，发给服务器
		this.getData();
	},
	computed: {
		// 这里不是对象了,直接写数组
	...
	methods: {
		...
		removeCategroyName() {
			// 将面包屑置空,还需要发送一次请求,并把参数置空,展示默认数据
			// 带给服务器的参数可有可无 属性值设置为空还是会把相应的字段带给服务器 但是undefined不带给服务器
			this.searchParams.categoryName = undefined;
			this.searchParams.category1Id = undefined;
			this.searchParams.category2Id = undefined;
			this.searchParams.category3Id = undefined;
			this.getData();
			// 地址栏也要修改,在当前路由自己跳自己,要带着params参数,删除query参数
			if (this.$route.params) {
				this.$router.push({ name: 'search', params: this.$route.params });
			}
		},
		removeKeyword() {
			// 给服务器的参数params的keyword置空\
			this.searchParams.keyword = undefined;
			this.getData();
			// 通知hader组件清空keyword
			this.$bus.$emit('clear');
			// 进行路由跳转,点击面包屑删除的时候,留着query参数
			if (this.$route.query) {
